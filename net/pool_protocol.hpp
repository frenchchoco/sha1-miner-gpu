...
namespace MiningPool {
    ...
    enum class ShareStatus { ACCEPTED, REJECTED_LOW_DIFFICULTY, REJECTED_INVALID, REJECTED_STALE, REJECTED_DUPLICATE };
    ...
    struct Share
    {
        ...
        // Calculated fields
        [[nodiscard]] double difficulty() const { return std::pow(2.0, matching_bits); }
    };
    ...
    struct PoolJob
    {
        ...
        [[nodiscard]] bool is_expired() const { return std::chrono::steady_clock::now() > expiry_time; }
    };
    ...
    public:
        ...
        static double bitsToScaledDifficulty(const uint32_t bits) { return std::pow(2.0, static_cast<double>(bits)); }

        // Format difficulty for display
        static std::string formatDifficulty(const double difficulty)
        {
            if (difficulty < 1e6)
                return std::to_string(static_cast<uint64_t>(difficulty));
            if (difficulty < 1e9)
                return std::to_string(static_cast<uint64_t>(difficulty / 1e6)) + "M";
            if (difficulty < 1e12)
                return std::to_string(static_cast<uint64_t>(difficulty / 1e9)) + "G";
